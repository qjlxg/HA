...
async def get_v2ray_links(session, url, max_pages=1, max_retries=3):
    ...
    while current_url and page_count < max_pages:
        ...
        while retry_count <= max_retries:
            try:
                await asyncio.sleep(1 * retry_count)  # 动态延迟避免速率限制
...
def save_configs_by_channel(configs, source_name):
    """保存代理配置到文件，并提取显示名称。"""
    valid_configs = [c for c in configs if is_valid_config(c)]
    ...
    source_names = [extract_channel_from_config(config) for config in valid_configs]
        ...
        with open(merged_file, 'r', encoding='utf-8') as infile:
            for line in infile:
                ...
                if not line_stripped or line_stripped.startswith('#'):
                    continue
                if re.match(protocol_pattern, line_stripped):
...
if __name__ == "__main__":
    ...
    source_stats = {name: 0 for name, _ in sources}
    active_sources = []
    inactive_sources = []
    source_name_map = {}
