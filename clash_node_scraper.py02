...
SEARCH_QUERIES = [
    ...
    '"proxies:" "clash" "ss" | "vmess" | "trojan" | "snell" | "vless" | "http" | "socks5"',
    '"proxy-providers:" "clash" | "subscribe" | "free node" | "free proxy"',
    '"clash" "proxy" | "node" site:*.herokuapp.com | site:*.pages.dev | site:*.vercel.app | site:*.workers.dev'
]
MAX_RESULTS_PER_QUERY = 50  # 减少每查询结果数，优化配额
PAGE_SIZE = 10
REQUEST_TIMEOUT = 8
MAX_RETRIES = 3
MAX_WORKERS = 30  # 增加并发上限
LINK_BLACKLIST = ['.md', '.png', '.jpg', '.pdf', '/issues/', '/wiki/', '/login', '/signup', '/readme']
...
def search_links():
    ...
    for query in SEARCH_QUERIES:
        ...
        print(f"查询 {query} 获取 {len(links)} 个链接")
