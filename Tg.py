...
async def get_v2ray_links(session, url, max_pages=3):
...
def merge_configs():
    """合并所有来源的配置到一个文件，并去重。"""
    config_folder = "sub"
    merged_file = "sub/merged_configs.txt"
    seen_configs = set()
                ...
                if filename.endswith('.txt') and filename not in ['merged_configs.txt', 'vless.txt', 'vmess.txt', 'trojan.txt', 'ss.txt', 'ssr.txt', 'hysteria2.txt']:
                    ...
                    try:
                        with open(file_path, 'r', encoding='utf-8') as infile:
                            ...
                            for line in infile:
                                if line.strip() and line not in seen_configs:
                                    outfile.write(line)
...
def split_configs_by_protocol():
    ...
    protocol_files = {
        'hysteria2': os.path.join(config_folder, 'hysteria2.txt'),
        'vmess': os.path.join(config_folder, 'vmess.txt'),
        'trojan': os.path.join(config_folder, 'trojan.txt'),
        'ss': os.path.join(config_folder, 'ss.txt'),
        'ssr': os.path.join(config_folder, 'ssr.txt'),
        'vless': os.path.join(config_folder, 'vless.txt'),
    }
...
def create_sub_section(source_name_map):
    ...
    for filename in sorted(os.listdir(sub_folder)):
        if filename.endswith('.txt') and filename not in ['merged_configs.txt', 'vless.txt', 'vmess.txt', 'trojan.txt', 'ss.txt', 'ssr.txt', 'hysteria2.txt']:
            ...
            url = f"https://raw.githubusercontent.com/qjlxg/ClashForge/main/sub/{urllib.parse.quote(source_name)}.txt"
            new_sub_section_lines.append(f"| [{display_name}]({url}) |")

    new_sub_section_lines.append(f"| [merged_configs](https://raw.githubusercontent.com/qjlxg/ClashForge/main/sub/merged_configs.txt) |")
    for protocol in ['hysteria2', 'vmess', 'trojan', 'ss', 'ssr', 'vless']:
        protocol_filename = f"{protocol}.txt"
        protocol_url = f"https://raw.githubusercontent.com/qjlxg/ClashForge/main/sub/{protocol_filename}"
...
if __name__ == "__main__":
    ...
    if sources_to_fetch:
        ...
        for source_name, configs in results:
            ...
            if isinstance(configs, list) and configs:
                ...
                if count > 0:
                    active_sources.append((source_name, sources_to_fetch[[n for n, (name, _) in enumerate(sources_to_fetch) if name == source_name][0]][1]))
    ...
    if any(count > 0 for count in source_stats.values()):
        ...
        print("所有任务完成：配置已保存，生成合并文件、协议文件、统计文件和 README.md。")
    else:
        print("\n所有来源均未获取到代理配置，未生成合并文件、统计文件或更新 README.md。")
