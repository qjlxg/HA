...
class CrawlerConfig:
    ...
    # max_crawl_depth_per_site: 在一个网站内部爬行的最大页面深度
    # 0 表示只抓取初始页面，1 表示抓取初始页面及其直接链接的页面，以此类推
...
def is_valid_node(node_url: str) -> bool:
    ...
    # name = node_info.get('name') # 名称不是校验的必要条件
    ...
    if scheme == "hysteria2":
        ...
        if not node_info.get('id'):
            ...
            return False
        # 可以进一步校验UUID格式，例如使用 re.match(r'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$', uuid, re.IGNORECASE)
        # 但通常不是强制的，因为UUID可以是任意字符串
        return True
        ...
        if not (node_info.get('cipher') and node_info.get('password')):
            ...
            return False
        # 可以添加对加密方法是否在已知列表中的校验
        return True

    elif scheme == "ssr":
        # SSR 校验：SSR 结构复杂，需要解析其内部参数
        # 确保 Base64 解码后能得到一些关键信息
        try:
            # 原始 SSR 链接的 netloc 部分是 Base64 编码的
            ...
            # SSR 内部结构通常是 server:port:protocol:method:obfs:base64_password/?params#name
            ...
            # 校验密码部分是否可解码 (如果存在)
            password_b64_part = parts[5].split('/?')[0].split('/#')[0] # 提取密码的Base64部分
            try:
                # 尝试解码，如果失败则认为无效
            ...
            # 进一步校验协议、加密方法、混淆等是否为非空字符串
            if not all(parts[i].strip() for i in [2, 3, 4]): # protocol, method, obfs
        ...
        # Vless 校验：确保有 UUID (在 parse_node_url_to_info 中被映射到 'id')
        if not node_info.get('id'):
...
def convert_dict_to_node_link(node_dict: Dict) -> Optional[str]:
    """
    将字典形式的节点数据转换为标准节点链接。
    此函数在 is_valid_node 之前被调用，主要用于将解析出的字典重新构建为URL。
    因此，它也需要进行一些必要的字段检查以避免构建无效链接。
    """
...
async def rename_and_deduplicate_by_geo(nodes: Set[str], config: CrawlerConfig) -> Set[str]:
    ...
    for detail in node_details:
        # --- 改进去重逻辑：基于标准化后的节点信息字典生成唯一哈希 ---
        # 复制节点信息，并移除不参与去重的字段
        normalized_info = detail['info'].copy()
        normalized_info.pop('original_url', None)
        normalized_info.pop('name', None) # 移除名称/备注，因为它不影响功能唯一性
        normalized_info.pop('ps', None) # 移除ps字段 (VMess备注)

        # 确保所有可变类型（如列表、嵌套字典）都被标准化为排序后的字符串
        # 递归地处理嵌套结构，确保它们也被排序和标准化
        def normalize_value(value):
            if isinstance(value, dict):
                return {k: normalize_value(v) for k, v in sorted(value.items())}
            elif isinstance(value, list):
                return [normalize_value(item) for item in sorted(value, key=str)]
            else:
                return value
        
        normalized_info = normalize_value(normalized_info)

        # 将标准化后的字典转换为排序的JSON字符串，然后计算哈希
        try:
            # separators=(',', ':') 移除空格，确保哈希一致性
            # ensure_ascii=False 允许非ASCII字符，但对于哈希通常不影响
            unique_json_str = json.dumps(normalized_info, sort_keys=True, separators=(',', ':'), ensure_ascii=False)
            unique_node_identifier = hashlib.sha256(unique_json_str.encode('utf-8')).hexdigest()
        except Exception as e:
            logger.warning(f"生成节点唯一标识符失败，使用原始URL哈希作为备用: {e} - {detail['original_url']}")
            unique_node_identifier = hashlib.sha256(detail['original_url'].encode('utf-8')).hexdigest()
        # --- 改进去重逻辑结束 ---
...
def is_same_domain(base_url: str, target_url: str) -> bool:
    """判断 target_url 是否与 base_url 处于同一主域名下（包括子域名）"""
    try:
        ...
        # 简单的子域名判断，例如 a.example.com 和 example.com
        if target_domain == base_domain:
            return True
        if target_domain.endswith(f".{base_domain}"):
            return True
        if base_domain.endswith(f".{target_domain}"): # base_domain 是 target_domain 的子域名
            return True
...
async def crawl_website(start_url: str, client: httpx.AsyncClient, semaphore: asyncio.Semaphore, 
    ...
    """
    异步爬行一个网站，发现内部链接并提取节点/新的订阅URL。
    返回 (该网站发现的所有节点, 该网站发现的所有新的订阅URL)
    """
    ...
    site_nodes = set() # 该网站发现的所有节点
    site_new_subscription_urls = set() # 该网站发现的所有新的订阅URL

    # 网站内部爬行队列和已访问集合
    site_queue = deque([(start_url, 0)]) # (url, current_site_crawl_depth)
    visited_pages_in_site = {start_url} # 跟踪该网站内部已访问的页面
    ...
    while site_queue:
        ...
        # 调用 process_url 来获取内容、提取节点和潜在URL
        # 注意：process_url 内部会处理缓存和保存单个页面的节点文件
        ...
        site_nodes.update(extracted_nodes) # 将当前页面提取的节点添加到网站总节点集合
        ...
        # 筛选 potential_new_urls:
        # 1. 如果是内部链接且未访问过，加入网站内部爬行队列
        # 2. 如果是外部订阅链接，加入网站发现的订阅URL集合
...
async def main():
    ...
    async with httpx.AsyncClient(timeout=config.request_timeout, verify=False) as client:
        ...
        # 主队列，用于跟踪待处理的订阅URL和网站入口URL
        main_queue = deque([(url, 0) for url in sources_urls]) # (url, current_overall_depth)
        urls_in_main_queue = set(sources_urls) # 跟踪已加入主队列的URL，避免重复

        while main_queue:
            current_url, current_overall_depth = main_queue.popleft() # 从队列左侧取出URL和当前深度
            ...
            try:
                # 判断当前URL是直接的订阅链接还是一个需要爬行的网站
                parsed_url = urlparse(current_url)
                # 简单判断是否为网站：没有明确的文件扩展名，或者以 / 结尾，或者包含 html
                ...
                if is_website and current_overall_depth < config.max_recursion_depth:
                    ...
                    unique_nodes.update(site_nodes) # 将网站内发现的节点添加到总节点集合

                    # 将网站内发现的新订阅URL添加到主队列，并增加深度
                ...
                await log_failed_url(current_url, f"主循环错误: {e}", config) # 记录失败URL
