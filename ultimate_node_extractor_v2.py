...
import base64
import yaml
import json # 用于处理缓存文件
import hashlib # 用于计算内容哈希
...
CACHE_FILE = os.path.join(DATA_DIR, "url_cache.json") # 用于存储URL内容哈希或ETag
...
USER_AGENTS = [
    ...
    # 移动设备 (Android Chrome, iOS Safari, iPhone Chrome)
    ...
    # 旧版浏览器 (兼容性考量，可能对某些旧网站有用)
    'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36',
    # 特定工具或机器人 (谨慎使用，可能被识别为爬虫)
    # 'curl/7.64.1',
...
def fetch_content(url, retries=3, cache_data=None):
    ...
    if cache_data and url in cache_data:
        if 'etag' in cache_data[url]:
            ...
            print(f"  使用 If-None-Match: {cache_data[url]['etag']}")
        if 'last_modified' in cache_data[url]:
            current_headers['If-Modified-Since'] = cache_data[url]['last_modified']
            print(f"  使用 If-Modified-Since: {cache_data[url]['last_modified']}")
    
    for attempt in range(retries):
        try:
            response = requests.get(url, timeout=20, headers=current_headers, allow_redirects=True)
            
            if response.status_code == 304: # Not Modified
                print(f"尝试 {attempt + 1}/{retries}: 内容未修改 (304 Not Modified) for {url}，跳过下载。")
                return None, None # 返回 None 表示内容未更新
                
            response.raise_for_status() # 对 4xx/5xx 状态码抛出 HTTPError
            print(f"尝试 {attempt + 1}/{retries}: 成功获取 {url}")
            ...
            print(f"尝试 {attempt + 1}/{retries}: 请求超时 for {url}")
        except requests.exceptions.RequestException as e:
            print(f"尝试 {attempt + 1}/{retries}: 获取失败 for {url} ({e})")
            
        # 如果原始 URL 失败，且是 HTTP，则尝试 HTTPS (但不对304进行此操作)
        if url.startswith("http://") and response.status_code != 304:
            https_url = url.replace("http://", "https://", 1)
            try:
                response = requests.get(https_url, timeout=20, headers=current_headers, allow_redirects=True)
                if response.status_code == 304:
                    print(f"尝试 {attempt + 1}/{retries}: HTTPS 内容未修改 (304 Not Modified) for {https_url}，跳过下载。")
                    return None, None
                response.raise_for_status()
                print(f"尝试 {attempt + 1}/{retries}: 成功通过 HTTPS 获取 {https_url}")
                
                new_etag = response.headers.get('ETag')
                new_last_modified = response.headers.get('Last-Modified')
                return response.text, {'etag': new_etag, 'last_modified': new_last_modified, 'content_hash': hashlib.sha256(response.text.encode('utf-8')).hexdigest()}
            except requests.exceptions.RequestException as e:
                print(f"尝试 {attempt + 1}/{retries}: HTTPS 获取失败 for {https_url} ({e})")
...
def decode_base64(data):
    ...
    data = data.strip()
    try:
        # Base64 解码需要输入是字节串，并且通常是 URL-safe 的 Base64
...
def is_valid_node(node_url):
    ...
    # ss/ssr/vmess 的主要信息在编码部分，直接检查 hostname 可能不准确
    ...
    # 可以添加更多针对特定协议的简单验证，例如trojan/vless必须有密码等
...
def parse_content(content):
    ...
    if decoded_content:
        ...
        if any(pattern.regex.search(decoded_content) for pattern in NODE_PATTERNS.values()):
            ...
            if 'proxies' in parsed_yaml and isinstance(parsed_yaml['proxies'], list):
                for proxy in parsed_yaml['proxies']:
                    if isinstance(proxy, dict) and 'type' in proxy:
                        ...
                        # 这是一个复杂且需要精确匹配 YAML 结构的部分
                        try:
                            if proxy['type'].lower() == 'vmess':
                                # Vmess 通常是 Base64 编码的JSON，这里需要重新编码整个代理配置
                                nodes_from_yaml.append(f"vmess://{base64.b64encode(json.dumps(proxy).encode('utf-8')).decode('utf-8')}")
                            elif proxy['type'].lower() == 'ss' and 'password' in proxy:
                                # Simple SS, base64(method:password)@server:port
                                method_pwd = f"{proxy.get('cipher')}:{proxy.get('password')}"
                                nodes_from_yaml.append(f"ss://{base64.b64encode(method_pwd.encode('utf-8')).decode('utf-8')}@{proxy.get('server')}:{proxy.get('port')}")
                            elif proxy['type'].lower() == 'trojan' and 'password' in proxy:
                                nodes_from_yaml.append(f"trojan://{proxy.get('password')}@{proxy.get('server')}:{proxy.get('port')}")
                            elif proxy['type'].lower() == 'vless' and 'uuid' in proxy:
                                # VLESS 构造可能更复杂，此处仅作示例
                                nodes_from_yaml.append(f"vless://{proxy.get('uuid')}@{proxy.get('server')}:{proxy.get('port')}")
                            elif proxy['type'].lower() == 'hysteria2' and 'password' in proxy:
                                nodes_from_yaml.append(f"hysteria2://{proxy.get('server')}:{proxy.get('port')}?password={proxy.get('password')}")
                            # 更多协议的 YAML 节点解析...
                        except Exception as e:
                            print(f"  警告: 解析 YAML 代理条目失败 ({proxy.get('type')}): {e}")
            
            # 将解析出的 YAML 节点与原始内容合并，以便后续正则表达式提取
            # 注意：这里假设正则表达式也能从原始YAML文本中匹配，或者YAML解析后重新构造的URL是新的
            return content + "\n" + "\n".join(nodes_from_yaml)
    except yaml.YAMLError:
        pass # 不是有效的 YAML

    # 3. 尝试 HTML 解析 (网页内容)
    # 如果包含 HTML 标签，则使用 BeautifulSoup 提取文本
    if '<html' in content.lower() or '<body' in content.lower() or '<!doctype html>' in content.lower():
        ...
        if soup.body:
            extracted_text.append(soup.body.get_text(separator="\n", strip=True))
...
def extract_and_validate_nodes(content):
    ...
    found_nodes = set() # 使用集合自动去重
    
    for pattern_name, pattern_regex in NODE_PATTERNS.items():
        matches = pattern_regex.findall(content)
        for match in matches:
            decoded_match = unquote(match).strip() # 解码 URL 编码
            if is_valid_node(decoded_match): # 再次进行有效性检查
                found_nodes.add(decoded_match)
            # else:
            #     print(f"排除无效节点 (is_valid_node): {decoded_match[:100]}...") # 用于调试
...
def load_existing_nodes(file_path):
    ...
    if os.path.exists(file_path):
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                for line in f:
                    # 从 "Proxy-001 = vmess://" 格式中提取原始节点URL
...
def main():
    ...
    all_new_and_existing_nodes = set(existing_nodes) # 初始化为已存在的节点
    url_node_counts = {}
    ...
    for url in source_urls:
        ...
        last_etag = url_cache.get(url, {}).get('etag')
