...
NODE_PATTERNS = {
    ...
    "ss": re.compile(r"ss://[a-zA-9\-\._~:/?#\[\]@!$&'()*+,;%=]+", re.IGNORECASE),
...
def fetch_content(url, retries=3, cache_data=None):
    ...
    for attempt in range(retries):
        for current_url_to_test in test_urls:
            try:
                # 调整：打印信息变为更简洁的，避免并发时日志混乱
                # print(f"  尝试请求: {current_url_to_test} (尝试 {attempt + 1}/{retries})")
                response = requests.get(current_url_to_test, timeout=REQUEST_TIMEOUT, headers=current_headers, allow_redirects=True, verify=False) # verify=False 忽略SSL证书错误，但降低安全性
                ...
                # print(f"  成功获取 {current_url_to_test}") # 成功获取也简化打印
...
def process_single_url(url, url_cache_data):
    ...
    if content is None and new_cache_meta is None:
        # print(f"  {url}: 内容未更新或获取失败。") # 简化日志，避免并发时刷屏
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        # 提交所有 URL 任务
        # 这里需要为每个 future 传递 url_cache 的一份副本，或者在一个锁的保护下更新
        # 为了简单起见，我们让 process_single_url 返回更新后的缓存元数据，最后再统一更新 main 里的 url_cache
