...
def process_links(working_links):
    ...
    for batch_idx, batch in enumerate(link_batches):
        ...
        for link, protocol in batch.items():
            full_url = f"{protocol}://{link}"
            # 优先尝试 HTML 解析
            tasks.append({'type': 'parse_and_fetch_from_html', 'url': full_url})
            # 仅在 HTML 解析失败时尝试 CONFIG_NAMES
            for config_name in CONFIG_NAMES:
                full_sub_url = f"{full_url}/{config_name}"
                if full_sub_url not in processed_links:
                    tasks.append({'type': 'fetch_yaml_content', 'url': full_sub_url})
        
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_task = {}
            for task in tasks:
                future = executor.submit(globals()[task['type']], task['url'])
